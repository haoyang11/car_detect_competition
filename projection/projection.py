...
def bird_view_proj(u, v, K, dist_coeff, R, T):
	...
	#print(undist_pt[0], " , ", undist_pt[1], "  ",  cx, " ", cy)
	# point on bird-view
	bv_x = fy / (undist_pt[1] - cy) * T[2] if v > cy + 1 else 10000
	bv_y = - (undist_pt[0] - cx) / fx * bv_x
	return bv_x, bv_y

def bird_view_proj_old(u, v, K, dist_coeff, R, T):
	"""
	:param u: horizontal pixel coordinate of ground point
	:param v: vertical pixel coordinate of ground point
	:param K: intrinsic parameters
...
def bird_view_proj_to_ground_point(X, Y, cam_param):	
	...
	for u in range(0, 1920, 39):
		for v in range(500 + 0, 1200 - 0, 39):
			#print u, " ", v
			x, y = bird_view_proj(u * 1.0, v * 1.0, K, dist_coeff, R, T)
			if coord_error((x, y), (X, Y)) < coord_error((x0, y0), (X, Y)):#if (x - X) ** 2 + (y - Y) ** 2 < (x0 - X) ** 2 + (y0 - Y) ** 2:
				x0, y0, u0, v0 = x, y, u, v
